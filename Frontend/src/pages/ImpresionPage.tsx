...
const getPrintBlockReason = (vale?: ValeDisponible) => {
  ...
  if (getVoucherDate(vale) !== toInputDate(now)) return 'Solo se pueden imprimir vales del dia actual.';
  if (isUsed(vale)) return 'Este vale ya fue utilizado.';
  if (isPrinted(vale)) return 'Este vale ya fue impreso.';
  if (vale.expirado || now > end) return 'Este vale esta expirado.';
...
const ImpresionPage = () => {
  ...
  const blockReason = printed ? 'Este vale ya fue impreso.' : getPrintBlockReason(vale);
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'No se pudo confirmar la impresion del vale.');
    } finally {
      setPrinting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background-light text-text-light">
      <Header title="Impresion de vale" />
        ...
        {!vale ? (
          ...
          <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-surface-light shadow-strong">
            <div className="bg-primary px-6 py-5 text-white">
              <p className="text-sm font-bold uppercase tracking-[0.18em] text-white/80">Simulacion de impresion</p>
              <h1 className="mt-2 text-3xl font-extrabold">Vale de colacion</h1>
            </div>
            ...
            <div className="bg-white px-6 py-7">
              <div className="mx-auto max-w-[320px] rounded-sm border border-slate-200 bg-slate-50 px-5 py-6 font-mono text-slate-800 shadow-inner">
                ...
                <div className="space-y-3 border-b border-dashed border-slate-300 py-4 text-sm">
                  ...
                  <ReceiptRow label="Codigo" value={user?.codigo || String(user?.id || '')} />
              ...
              {printed && <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">Vale marcado como impreso.</div>}
              ...
              <div className="mt-6 grid grid-cols-1 gap-3">
                ...
                >
                  ...
                  {printing ? 'Imprimiendo...' : blockReason ? 'No imprimible' : 'Confirmar impresion'}
                </button>
